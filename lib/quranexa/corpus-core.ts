...
async function source(id:string):Promise<Source|null>{
 ...
 const inlineHadith=hadith.find(s=>s.id===id);if(inlineHadith)return inlineHadith;
...
async function search(query:string,kind='all',limit=60):Promise<Source[]>{
 ...
 const local=kind==='tafsir'?[]:searchSources(query,Math.max(limit,100)).filter(s=>kind==='all'||s.kind===kind);
 if(kind==='quran'||kind==='dua'||kind==='hadith')return local.slice(0,limit);
 ...
 if(!tokens.length)return local.slice(0,limit);
 ...
 const ids=await reader<string[]>('search/ids.json');if(!ids)return local.slice(0,limit);const ranked=[...scores.entries()].filter(([id,score])=>score>=Math.max(1,Math.ceil(tokens.length*.5))&&(kind==='all'||ids[id]?.startsWith(kind+':'))).sort((a,b)=>b[1]-a[1]);const selected=kind==='all'?[...ranked.filter(([id])=>ids[id].startsWith('hadith:')).slice(0,20),...ranked.filter(([id])=>ids[id].startsWith('tafsir:')).slice(0,12)]:ranked.slice(0,Math.min(limit,40));const found:Source[]=[];
 ...
 const seen=new Set<string>();return [...(kind==='all'?[...local.filter(s=>s.kind==='quran').slice(0,24),...local.filter(s=>s.kind==='dua').slice(0,12),...local.filter(s=>s.kind==='hadith').slice(0,20)]:local),...found].filter(s=>{if(seen.has(s.id))return false;seen.add(s.id);return true}).slice(0,limit);
}
async function collectionPage(col:string,book?:string,page=1){const c=catalog.find(c=>c.id===col);if(!c)return null;const entries=(book?c.books.filter(b=>b.id===book):c.books).flatMap(b=>b.ids);const start=(page-1)*20;let records=(await Promise.all(entries.slice(start,start+20).map(id=>source(`hadith:${col}:${id}`)))).filter(Boolean) as Source[];let total=entries.length;
 if(!records.length){const inline=hadith.filter(h=>h.collection===col&&(!book||String(h.bookId||h.reference?.book||'')===book));total=inline.length;records=inline.slice(start,start+20);}
 return {collection:c,records,total,page,pages:Math.max(1,Math.ceil(total/20))};}
